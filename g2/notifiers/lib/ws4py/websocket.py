...
class WebSocket(object):
    ...
    def _write(self, b):
        ...
        sendrc = self.sock.sendall(b)
        logger.debug("Sent message of len=%d with rc=%d:\n%s", len(b), sendrc,
                     b if all(ord(c) < 128 for c in b) else ":".join("%02x"%ord(c) for c in b))
    ...
    def once(self):
        ...
        try:
            ...
            if len(b) and self._is_secure:
                b += self._get_from_pending()
            logger.debug("Got message of len %d", len(b))
