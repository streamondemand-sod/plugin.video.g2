...
def menu(**kwargs):
    if dbs.url('movies_recently_added{}'):
        ...
        ui.addDirectoryItem(_('Latest Movies')+' ['+db_provider.upper()+']', 'movies.movielist&url='+dbs.url('movies_recently_added{}', quote_plus=True), 'moviesAdded.jpg', 'DefaultRecentlyAddedMovies.png')
    ui.addDirectoryItem(_('Search by Title'), 'movies.search', 'movieSearch.jpg', 'DefaultMovies.png')
    ui.addDirectoryItem(_('Search by Person'), 'movies.person', 'moviePerson.jpg', 'DefaultMovies.png')
    ui.addDirectoryItem(_('Search by Year'), 'movies.year', 'movieYears.jpg', 'DefaultMovies.png')
    ...
    if dbs.url('movies_featured{}'):
        ui.addDirectoryItem(_('Featured'), 'movies.movielist&url='+dbs.url('movies_featured{}', quote_plus=True), 'movies.jpg', 'DefaultRecentlyAddedMovies.png')
    if dbs.url('movies_trending{}'):
        ui.addDirectoryItem(_('People Watching'), 'movies.movielist&url='+dbs.url('movies_trending{}', quote_plus=True), 'moviesTrending.jpg', 'DefaultRecentlyAddedMovies.png')
    if dbs.url('movies_popular{}'):
        ui.addDirectoryItem(_('Most Popular'), 'movies.movielist&url='+dbs.url('movies_popular{}', quote_plus=True), 'moviesPopular.jpg', 'DefaultMovies.png')
    if dbs.url('movies_toprated{}'):
        ui.addDirectoryItem(_('Most Voted'), 'movies.movielist&url='+dbs.url('movies_toprated{}', quote_plus=True), 'moviesViews.jpg', 'DefaultMovies.png')
    if dbs.url('movies_boxoffice{}'):
        ui.addDirectoryItem(_('Box Office'), 'movies.movielist&url='+dbs.url('movies_boxoffice{}', quote_plus=True), 'moviesBoxoffice.jpg', 'DefaultMovies.png')
    if dbs.url('movies_oscar{}'):
        ui.addDirectoryItem(_('Oscar Winners'), 'movies.movielist&url='+dbs.url('movies_oscar{}', quote_plus=True), 'moviesOscars.jpg', 'DefaultMovies.png')
    if dbs.url('movies_theaters{}'):
        ui.addDirectoryItem(_('In Theaters'), 'movies.movielist&url='+dbs.url('movies_theaters{}', quote_plus=True), 'moviesTheaters.jpg', 'DefaultRecentlyAddedMovies.png')
    ui.endDirectory()
...
def userlists(action, **kwargs):
    ...
    if _trakt_user:
        ...
        for i in trakt_items:
            i.update({
                'action': 'movies.movielist',
                'url': dbs.url('movies{trakt_user_id}{trakt_list_id}', trakt_user_id=_trakt_user, trakt_list_id=i['trakt_list_id']),
            })
...
def _fetch_movie_info(items):
    ...
    if metas:
        ...
        for m, i in zip(metas, items):
            if m['item']:
                i.update(dict((k,v) for k, v in m['item'].iteritems()
                                        if k not in ['fanart', 'rating']
                                        or (k in ['fanart', 'rating'] and i.get(k, '0') == '0')))
...
def _add_movie_directory(action, items):
    ...
    for i in items:
        try:
            ...
            url = '%s?action=sources.dialog&title=%s&year=%s&imdb=%s&tmdb=%s&meta=%s' % (_sysaddon, systitle, year, imdb, tmdb, sysmeta)
            ...
            if platform.condition('System.HasAddon(script.extendedinfo)'):
                ...
                cm.append((_('Movie information'), 'Action(Info)'))

            cm.append((_('Play items as folder once'), 'RunPlugin(%s?action=sources.usefolderonce)' % (_sysaddon)))
            if watched:
                cm.append((_('Mark as unwatched'), 'RunPlugin(%s?action=movies.unwatched&imdb=%s)' % (_sysaddon, imdb)))
            else:
                cm.append((_('Mark as watched'), 'RunPlugin(%s?action=movies.watched&imdb=%s)' % (_sysaddon, imdb)))

            cm.append((_('Clear sources cache'), 'RunPlugin(%s?action=sources.clearsourcescache&imdb=%s)' % (_sysaddon, imdb)))
    ...
    if len(items) and 'next_action' in items[0]:
        ui.endDirectory(content='movies', next_action=items[0]['next_action'], next_url=items[0]['next_url'], next_page=items[0]['next_page'])
    else:
...
def _add_directory(action, items, content=None):
    ...
    for i in items:
        try:
            ...
            elif artPath: thumb = os.path.join(artPath, i['image'])
            ...
            if content == 'movies':
                if 'genre' in i:
                    ...
                    item.setProperty('Video', 'true')
                if 'poster' in i:
                    if i['poster'].startswith('http://'): poster = i['poster']
                    elif artPath: poster = os.path.join(artPath, i['poster'])
    ...
    if len(items) and 'next_action' in items[0]:
        ui.endDirectory(content=content, next_action=items[0]['next_action'], next_url=items[0]['next_url'], next_page=items[0]['next_page'])
    else:
